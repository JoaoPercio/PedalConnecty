...
export default async function PedalDetailPage({ params }: PedalPageProps) {
  ...
  const { data, error } = await supabase
    .from("pedals")
    .select(
      "id,creator_id,name,description,date,status,started_at,ended_at,distance_km,elevation_gain,difficulty,terrain,max_participants,requires_safety_equipment,required_equipment,age_group,visibility,start_location,start_lat,start_lng,route_geojson,route_waypoints"
    )
    .eq("id", id)
    .single();
