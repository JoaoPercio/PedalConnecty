-- Join-request notification to pedal owner (creator): message includes requester name + pedal name;
-- data jsonb also carries from_user_name and pedal_name for the UI.
...
begin
  ...
  select nullif(
    trim(both from concat_ws(' ', nullif(trim(pr.first_name), ''), nullif(trim(pr.last_name), ''))),
    ''
  )
  into v_user_name
  ...
  v_user_name := coalesce(v_user_name, 'Alguém');
  ...
  if new.user_id is distinct from v_creator then
    if (tg_op = 'INSERT' and new.status = 'pending')
      ...
      values (
        ...
        jsonb_build_object(
          'pedal_id', new.pedal_id,
          'from_user_id', new.user_id,
          'from_user_name', v_user_name,
          'pedal_name', v_pedal_name
        ),
        now() + interval '2 days'
      );
      return new;
